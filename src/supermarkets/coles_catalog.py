...
# The full set a browser sends on a top-level navigation. The previous three
# headers were an incomplete request, and an incomplete request is what a
# script sends -- so it was treated as one.
_HEADERS = {
    "User-Agent": (
...
def search(query: str, limit: int = 10, session: Optional[Any] = None) -> Dict[str, Any]:
    ...
    # Whether this is the real page is decided by whether the page data is
    # present -- not by looking for Imperva's script, which appears on pages it
    # has allowed as much as on ones it is challenging. Testing for the script
    # threw away good responses.
    body = response.text
    if "__NEXT_DATA__" not in body:
        kind = ("Coles answered with a JavaScript challenge instead of the "
                "page." if "_Incapsula_Resource" in body
